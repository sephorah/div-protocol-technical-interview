...
def update_distinct_elements_list(map: list[list[str]], i: int, j: int, elem_list: list[list[tuple[int, int]]]) -> None:
    if (i, j) not in elem_list:
        elem_list.append((i,j))

def move_forward(map: list[list[str]], i: int, j: int, rows: int, columns: int, distinct_positions: list[int]) -> tuple[int, int]:
    if map[i][j] == "^" and not is_out_of_bounds(i - 1, j, rows, columns) and \
        map[i - 1][j] != "#":
...
def rotate(map: list[list[str]], i: int, j: int):
    if map[i][j] == "^":
        map[i][j] = ">"
    elif map[i][j] == ">":
        map[i][j] = "v"
